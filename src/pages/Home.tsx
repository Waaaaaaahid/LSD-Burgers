...
import { ArrowRight, Flame, Truck, Clock, UtensilsCrossed, Sandwich, Drumstick, CookingPot, Popcorn, Fish, CupSoda, Package, Boxes, Layers3, Utensils } from 'lucide-react';
...
const CATEGORY_ICONS: Record<string, typeof Sandwich> = {
  burgers: Sandwich,
...
export function Home() {
  ...
  return <div className="animate-fade-in">
    ...
    <section className="bg-white py-16"><div className="max-w-7xl mx-auto px-4 sm:px-6"><div className="mb-8 text-center"><p className="section-eyebrow mb-1">Browse</p><h2 className="font-display text-4xl sm:text-5xl tracking-tight text-lsd-gray-900">Categories</h2></div><div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-8 gap-3">{categories.map(cat=>{const Icon=CATEGORY_ICONS[cat.slug]||UtensilsCrossed; return <button key={cat.id} onClick={()=>navigate(`/menu?cat=${cat.slug}`)} className="card card-hover group p-4 flex flex-col items-center gap-2.5"><span className="w-12 h-12 rounded-2xl bg-lsd-blue/5 border border-lsd-blue/10 flex items-center justify-center group-hover:bg-lsd-blue group-hover:border-lsd-blue transition-colors"><Icon className="w-6 h-6 text-lsd-blue group-hover:text-white transition-colors" strokeWidth={1.8}/></span><span className="text-xs font-semibold text-lsd-gray-700 text-center leading-tight">{cat.name}</span></button>})}</div></div></section>
